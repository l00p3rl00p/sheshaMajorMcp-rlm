...
### Option B: Operator GUI (Recommended)
```bash
librarian bridge
librarian gui
```
Open the Dashboard, select your project, and ask away!

### What to expect:
1. **Thought Loop**: You'll see Shesha "thinking" as it decides which files to read.
2. **Execution**: It will run code to analyze the logic.
...
3. Start the Bridge: `librarian bridge`.
4. Open the GUI: `librarian gui` OR run your commands (`query`, `ingest`, etc.).

### How do I gracefully shut down?
*   **Bridge/GUI**: Use `Ctrl+C` in the terminal where `librarian bridge` is running.
