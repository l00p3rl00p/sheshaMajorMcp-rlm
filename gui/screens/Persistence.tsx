...
import { BridgeClient, ManifestStatus } from '../src/api/client';
...
export const PersistenceScreen: React.FC<Props> = ({ onNavigate, currentScreen }) => {
  ...
  const [manifestData, setManifestData] = useState<ManifestStatus | null>(null);
  const [isLoadingManifest, setIsLoadingManifest] = useState(true);

  useEffect(() => {
    const fetchManifest = async () => {
      setIsLoadingManifest(true);
      const data = await BridgeClient.getManifest();
        ...
        <div className="flex px-4 justify-between gap-4">
          {(['manifest', 'storage', 'logs'] as const).map(tab => (
            <button
              ...
              className={`flex flex-col items-center justify-center border-b-[3px] pb-3 flex-1 transition-all capitalize ${activeTab === tab ? 'border-primary text-white' : 'border-transparent text-gray-500 hover:text-white'
                }`}
              title={`Show ${tab} view`}
            >
        ...
        {activeTab === 'manifest' && (
          <div className="pt-6 animate-fade-in">
            <div className="flex items-center justify-between px-4 pb-3">
              ...
              {manifestData && (
                <span className="text-xs font-mono text-text-secondary bg-surface-dark px-2 py-1 rounded border border-border-dark">
                  {manifestData.expected_path}
                </span>
              )}
            </div>
            ...
            {isLoadingManifest ? (
              ...
              <div className="mx-4 mb-6 rounded-xl border border-red-500/30 bg-surface-dark overflow-hidden">
                <div className="flex items-center gap-3 p-4">
                  <AlertCircle size={24} className="text-red-500" />
                  <div>
                    <p className="text-white font-bold">Manifest Not Found</p>
                    <p className="text-text-secondary text-sm">Expected at: {manifestData?.expected_path}</p>
                    <p className="text-text-secondary text-xs mt-1">Run <code className="bg-border-dark px-1 rounded">librarian install</code> to create it.</p>
                  </div>
                </div>
              </div>
            ) : (
              <div className="mx-4 mb-6 rounded-xl border border-border-dark bg-surface-dark overflow-hidden">
                ...
                <div className="p-4 grid grid-cols-[35%_1fr] gap-x-4 gap-y-0 font-mono text-xs sm:text-sm">
                  {/* Real Manifest Data */}
                  {[
                    ['manifest.exists', String(manifestData.exists), 'text-primary'],
                    ['manifest.valid', String(manifestData.valid), manifestData.valid ? 'text-green-400' : 'text-red-400'],
                    ['manifest.path', manifestData.path || 'N/A', 'text-blue-400'],
                    ['manifest.configured', String(manifestData.configured), manifestData.configured ? 'text-green-400' : 'text-amber-300'],
                    ['manifest_dir', manifestData.manifest_dir, 'text-orange-300']
